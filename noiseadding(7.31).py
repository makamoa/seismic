...
#         self.noisemean=noisestd
#     def __call__(self, sample):

#         input, target = sample['input'], sample['target']
#         print(input.shape,target.shape)
#         img=input.squeeze()
#         noise =  np.random.normal(loc=0, scale=self.noisemean, size=img.shape)
#         input=np.expand_dims(img+noise,axis=2)
...
class add_spnoise(BaseNoise):
    def __init__(self,  ampl_mu=0.005, ampl_std=1e-5,per_mu=1,per_std=2e-1):
        self.mu = ampl_mu
        self.std = ampl_std
        self.per_mu=per_mu
    ...
    def __call__(self, sample):
        ...
        coords = [np.random.randint(0, i - 1, int(num_pepper))
                  for i in img.shape]
        out[coords] = 0
        input = np.expand_dims(((out-img)*noisescale*0.35+img), axis=2)
        return {'input': input, 'target': target}
...
class add_specklenoise(BaseNoise):
    ...
    def __call__(self, sample):
        ...
        noisy = img + img * gauss * noisescale *0.06
        input = np.expand_dims(noisy, axis=2)
        return {'input': input, 'target': target}


class add_noise_FFT(BaseNoise):
    def __init__(self, masktype="crossfilter", ampl_mu=0.95, ampl_std=1e-5,frac_mu=0.01,frac_std=1e-23, flow_mu=80, flow_std=1e-13,fhigh_mu=0.2, fhigh_std=1e-9, per_mu=5,per_std=2e-1):
...
#                 print(D0)
    ...
    #                     base[y,x] = exp(((-distance((y,x),center)**2)/(2*(D0**2))))
                    ...
                    base[y,x] =exp(((-self.distance((y,x),center)**2)/(2*(D0**2)))*150)
...
#             print(keep_fraction)
            ...
            purenoise=(im_new-img)*1e10

        elif masktype == "addrandomnoise":
            im_fft2 = im_fft.copy()
            im_fft2 = im_fft2 + np.random.normal(loc=0, scale=noiselevel, size=np.abs(im_fft2).shape)*100
            im_new = fftpack.ifft2(im_fft2).real
...
#             d0 = f_lowpas
            #             print(d0)
            LowPassCenter = center * self.gaussianLP(img.shape)
            LowPass = np.fft.ifftshift(LowPassCenter)
            inverse_LowPass = np.fft.ifft2(LowPass)
            im_new=inverse_LowPass.real
#         LowPassCenter = center * gaussianLP(d0,img.shape)
#         np.fft.ifftshift(LowPassCenter)
#             im_fft2 = np.fft.ifftshift(LowPassCenter)
#             im_new = fftpack.ifft2(im_fft2).real
#             purenoise=(im_new-img)*5
...
class add_linearnoise(BaseNoise):
    ...
    def __call__(self, sample):
        ...
        slope = np.random.uniform(low = self.slope_mu - sqrt(3)*self.slope_std, high = self.slope_mu + sqrt(3)*self.slope_std)
...
class add_hyperbolic_noise(BaseNoise):
    ...
    def __init__(self,  v_mu=1000, v_std=1e-1,
                        tsample_mu=0.05, tsample_std=1e-3,                     
                        ampl_mu=0.00005, ampl_std=0.000005):
        self.v_mu = v_mu
        self.v_std = v_std
        self.tsample_mu = tsample_mu
        self.tsample_std = tsample_std
...
#         tsample = self.tsample
        ...
        for j in range(num_events):
            ...
            input += np.expand_dims(y, axis=2) *noiselevel*0.12
...
class CombinedTransforms():
    ...
    def __call__(self, sample):
        noises = [transform.__shifted_call__(sample)['input'] * scale for transform, scale in zip(self.transforms, self.scale)]
        noise = sum(noises)
        self.signal_to_noise = 1 / noise.max()
        print(self.signal_to_noise)
...
hyperbolic_noise=[add_hyperbolic_noise(ampl_mu=9.5, ampl_std=1.0)]

addcro_noise_FFT=[add_noise_FFT( masktype="crossfilter")]
addcran_noise_FFT=[add_noise_FFT( masktype="addrandomnoise")]
adhigh_noise_FFT=[add_noise_FFT( masktype="highpassfilter",fhigh_mu=100)]
addlow_noise_FFT=[add_noise_FFT( masktype="lowpassfilter")]
addsp_noise_FFT=[add_noise_FFT( masktype="adds&pnoise",ampl_mu=0.005, ampl_std=1e-5,per_mu=1,per_std=2e-1)]

gaussian_color_noise = gaussian_noise + color_noise
complex_noise_transforms = gaussian_noise + color_noise + linear_noise+blurnoise+bandpassed_noise+spnoise+specklenoise+hyperbolic_noise+addcran_noise_FFT+addlow_noise_FFT+addsp_noise_FFT+adhigh_noise_FFT+addcro_noise_FFT
#
...
#
#
# gaussian_color = [add_gaussnoise(noisestd=np.random.uniform(low=5, high=10, size=None)),
#                   add_color_noise(noisescale=np.random.uniform(low=0.5, high=1.0, size=None))]
#
# gaussian_color_linear = [add_gaussnoise(noisestd=np.random.uniform(low=5, high=10, size=None)),
#                           add_color_noise(noisescale=np.random.uniform(low=0.5, high=1.0, size=None)),
#                          add_linearnoise(v=np.random.uniform(low=100, high=120, size=None),
#                                          tsample=np.random.uniform(low=0.05, high=0.08, size=None),
#                                          slope=np.random.uniform(low=5, high=10, size=None),
#                                          ampli=np.random.uniform(low=5.5, high=6.0, size=None))
#                          ]
#
#
# gaussian_color_linear_fft = [add_gaussnoise(noisestd=np.random.uniform(low=5, high=10, size=None)),
#                           add_color_noise(noisescale=np.random.uniform(low=0.5, high=1.0, size=None)),
#                          add_linearnoise(v=np.random.uniform(low=100, high=120, size=None),
#                                          tsample=np.random.uniform(low=0.05, high=0.08, size=None),
#                                          slope=np.random.uniform(low=5, high=10, size=None),
#                                          ampli=np.random.uniform(low=5.5, high=6.0, size=None)),
#                           add_bandpassed_noise(f_low=np.random.uniform(low=1, high=5, size=None),
#                                                f_high=np.random.uniform(low=8, high=10, size=None),
#                                                noisescale=np.random.uniform(low=0.5, high=1.0, size=None))
#
#                          ]
#
# gaussian_color_linear_fft_hyperbolic = [add_gaussnoise(noisestd=np.random.uniform(low=5, high=10, size=None)),
#                              add_color_noise(noisescale=np.random.uniform(low=0.5, high=1.0, size=None)),
#                              add_linearnoise(v=np.random.uniform(low=100, high=120, size=None),
#                                              tsample=np.random.uniform(low=0.05, high=0.08, size=None),
#                                              slope=np.random.uniform(low=5, high=10, size=None),
...
#                                                   f_high=np.random.uniform(low=8, high=10, size=None),
#                                                   noisescale=np.random.uniform(low=0.5, high=1.0, size=None)),
#                                         add_hyperbolic_noise(v=np.random.uniform(low=10, high=20, size=None),
#                                                              tsample=np.random.uniform(low=0.05, high=0.08, size=None),
#                                                              noisedivide=np.random.uniform(low=5, high=8, size=None),
#                                                              ampli=np.random.uniform(low=0.05, high=1.0, size=None))
#                                         ]
#
#
