...
def get_first_break_dataset(rootdir="/data/maksim/data1/",
                       noise_transforms=[]):
    transforms_ = []
    transforms_ += [add_linearnoise (v=10,tsample=0.1,slope=1,ampli=10)]
    transforms_ += [add_color_noise(noisescale=0.005),add_blurnoise(ksizemax=3),add_bandpassed_noise(f_low=1,f_high=10,noisescale=0.5),add_rainnoise(drop_length=20,drop_width=1,drop_color=(1)),add_spnoise(percentage=10)]
    transforms_ += [add_gaussnoise(noiselevel=3),add_specklenoise(noiselevel=0.2)]
    transforms_ += [add_noise_FFT(masktype="highpassfilter",keep_fraction=0.3,noiselevel=0.3,f_lowpas=80,f_high=10,percentage=5)]
    transforms_ += [add_noise_FFT(masktype="lowpassfilter",keep_fraction=0.3,noiselevel=0.3,f_lowpas=80,f_high=10,percentage=5)]
    transforms_ += [add_noise_FFT(masktype="crossfilter",keep_fraction=0.3,noiselevel=0.3,f_lowpas=80,f_high=10,percentage=5)]
    transforms_ += [add_noise_FFT(masktype="addrandomnoise",keep_fraction=0.3,noiselevel=0.3,f_lowpas=80,f_high=10,percentage=5)]
    transforms_ += [add_noise_FFT(masktype="adds&pnoise",keep_fraction=0.3,noiselevel=0.3,f_lowpas=80,f_high=10,percentage=5)]
